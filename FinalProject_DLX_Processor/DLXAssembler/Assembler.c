...
int main(int argc, char* argv[]) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (data_flag == 2) {
            // Const format is: <label_name> <size> "string content"
            // Each character is stored as a 32-bit word at its own address.
            char* label_name = strdup(token);

            // Add this const label to our list of all labels.
            ...
            // Parse the size (number of characters).
            token = strtok(NULL, " ,\t\n\r");
            if (token == NULL) continue;
            int size = (int)strtol(token, NULL, 0);

            // Find the opening quote in the original line to extract the string.
            // (buffer is modified by strtok, so search the original line instead.)
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (opcode_idx != -1) {
            ...
            // Print Instructions (e.g., PCH rs1, PD rs1, PDU rs1)
            // Format: Op[31:26] | Rs1[25:21] | Unused[20:0]
            else if (strcmp(op_name, "PCH") == 0 || strcmp(op_name, "PD") == 0 || strcmp(op_name, "PDU") == 0) {
                char* rs1_str = strtok(NULL, " ,\t\n\r()");
                int rs1 = get_register(rs1_str);
                instruction |= (rs1 & 0x1F) << 21;
            }
